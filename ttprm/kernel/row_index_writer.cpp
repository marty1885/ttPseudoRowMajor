// Writer endpoint for indexed whole-tile copies.
// ROW_INDEXED scatters into destination row idx[ord/T]; dense mode writes page
// ordinals directly.
//
// Batched issue: wait K tiles, emit K writes, use one barrier, then pop K.
//
// Compile args: dst TensorAccessorArgs [, idx TensorAccessorArgs if ROW_INDEXED].
// Runtime args: 0 dst_addr  1 count  2 t0  3 T  4 K  [5 idx_addr  6 idx_pages  7 n_idx_rows].
//
// Bounds: invalid indices skip the write, leaving the destination row unchanged.
...
void kernel_main() {
    ...
    constexpr auto idx_args = TensorAccessorArgs<dst_args.num_compile_time_args()>();
    ...
    for (uint32_t k = 0; k < count; k += K) {
        ...
        cb_wait_front(cb_io, chunk);
        const uint32_t rp = get_read_ptr(cb_io);
        for (uint32_t j = 0; j < chunk; j++) {
            const uint32_t ord = t0 + k + j;
#ifdef ROW_INDEXED
            const uint32_t row = ord / T;
            const uint32_t ri  = (uint32_t)I[row];
            if (ri >= n_idx_rows) continue;
            const uint32_t page = ri * T + (ord - row * T);
#else
            const uint32_t page = ord;
#endif
            noc_async_write(rp + j * tile_bytes, dst.get_noc_addr(page, 0), tile_bytes);
        }
        noc_async_write_barrier();
        cb_pop_front(cb_io, chunk);
    }
}
